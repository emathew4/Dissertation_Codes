function [dcmhdr] = philips_dcm2nii_dce(dcmfile)
...
for ff=1:num_dcmfiles % loop for all the files
    ...
    Ndyn      = dcmhdr.Private_2001_1081;
    Nechos    = dcmhdr.Private_2001_1082;
    ...
    tmp = zeros(size(data,1),size(data,2),Nslices,Nechos,Ndyn,'uint16');
    for ss=slc
        for dd=dyn
            for ee=echo
                tmp(:,:,ss,ee,dd) = data(:,:,ee+(dd-1)*Nechos+(ss-1)*Ndyn*Nechos);
            end
        end
    end
    ...
    %data=reshape_philips_dki(data,'NumSlices',Nslices,'NumBVal',numBVal);
