function [pvals,significantpairs] = wt_als_tp_hist_boxplots(wtcell,alscell,category,categorynames,colors,legendnames)
...
% wt_als_tp_hist_boxplots({wt70m,wt110m,wt150m,wt170m},{als70m,als110m,als150m,als170m},[1,2,3,4,1,2,3,4],{'Day70','Day110','Day150','Day170'},[col;col;]',{'WT Male','ALS Male'});
% wt_als_tp_hist_boxplots({wt70f,wt110f,wt150f,wt170f},{als70f,als110f,als150f,als170f},[1,2,3,4,1,2,3,4],{'Day70','Day110','Day150','Day170'},[col;col;]',{'WT Female','ALS Female'});
...
s=fieldnames(wtcell{1});

pvals=zeros(1,length(s));
significantpairs={};
for fnames=1:length(s)
    x=[];
    g=[];
    currnum=1;
    for i=1:length(u)
        ...
        for j=1:length(currcat)
            ...
            %newv=currcat{j}.(s{fnames});

            if strcmp('emhyc',s{fnames}) || strcmp('density',s{fnames})
            x=[x;currcat{j}.(s{fnames})'];
            g=[g;currnum*ones(size(currcat{j}.(s{fnames})'))];
            else
            x=[x;mean(currcat{j}.(s{fnames}),'omitnan')'];
            g=[g;currnum*ones(size(mean(currcat{j}.(s{fnames}),'omitnan')'))];
            end
...
%% Boxplots
    ...
    %grouped_boxplots(x,g,sort(category),categorynames,colors,legendnames,true);
    positions = grouped_boxplots(x,g,sort(category),categorynames,colors,legendnames);
    %grouped_violinplots(x,g,sort(category),categorynames,colors,legendnames,true);
